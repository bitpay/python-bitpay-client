...
date_start = (date.today() + timedelta(days=3)).strftime("%Y%m%d")

payout = Payout(5.0, Currency.USD, Currency.ETH)
payout.set_recipient_id("PnPkYQ1msbZS9etHu5kuXu")
payout.set_notification_url("https://hookb.in/wNDlQMV7WMFz88VDyGnJ")

submit_payout = bitpay.submit_payout(payout)
print(submit_payout.get_id())
```
...
today = date.today().strftime("%Y%m%d")
date_start = (date.today() + timedelta(days=3)).strftime("%Y%m%d")
payouts = bitpay.get_payout(today, date_start, PayoutStatus.New, None, None, None);
```
