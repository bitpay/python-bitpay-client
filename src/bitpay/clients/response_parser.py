...
class ResponseParser:
    ...
    def response_to_json_string(response: Response) -> Any:
        ...
        if not response.json():
            BitPayExceptionProvider.throw_api_exception_with_message("HTTP response is null")

        response_obj = response.json()

        request = response.request
        LoggerProvider.get_logger().log_response(request.method, request.url, response_obj)

        if "status" in response_obj:
            if response_obj["status"] == "error":
                BitPayExceptionProvider.throw_api_exception_with_message(response_obj["error"], response_obj["code"])

        if "error" in response_obj:
            BitPayExceptionProvider.throw_api_exception_with_message(response_obj["error"])
